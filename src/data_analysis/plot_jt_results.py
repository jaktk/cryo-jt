#!/usr/bin/env python3
"""
Plot Joule-Thomson coefficient results for thesis figures.

This script generates publication-quality plots of JT coefficient measurements
with error bars, similar to figures in PhD thesis Chapter 4.

Updated to work with current directory structure and calculated JT coefficients.
"""
...
class JTPlotter:
    """Class for plotting JT coefficient results."""
    ...
    def _plot_isenthalpic_line(self, ax, fluid_props, p_range, h_value, **kwargs):
        """Plot theoretical isenthalpic line."""
        try:
            temperatures = []
            for p in p_range:
                try:
                    T = fluid_props.get_T(p, h_value)
            ...
            p_valid = p_range[valid_mask]
            T_valid = np.array(temperatures)[valid_mask]
            
            if len(p_valid) > 0:
                ax.plot(p_valid, T_valid, **kwargs)
        except Exception as e:
            print(f"Error plotting isenthalpic line: {e}")
    
    def plot_pure_fluids_with_errorbars(self, save_figure=True):
        """
        Plot pure fluid measurements with error bars.
        
        Creates figure similar to thesis fig:meas_pures_errbar
        """
    ...
    def plot_helium_neon_with_errorbars(self, save_figure=True):
        ...
        Creates figure similar to thesis fig:meas_HeNe_errorbar
        """
    ...
    def _plot_fluid_measurements(self, ax, fluid_data, fluid_name, colors):
        ...
        # Initialize FluidProps for theoretical calculations
        if fluid_name == 'Helium-Neon':
            fluid_props = FluidProps(('Helium', 'Neon'))
        elif fluid_name == 'Helium-Nitrogen':
            fluid_props = FluidProps(('Helium', 'Nitrogen'))
        else:
            fluid_props = FluidProps(fluid_name)
        ...
        for i, data in enumerate(fluid_data):
            ...
            # Set composition for mixtures
            if data.get('composition') is not None:
                fluid_props.set_composition_from_1st_fraction(data['composition'])
            ...
            if len(p_data) > 2:
                try:
                    # Get enthalpy at highest pressure point
                    h_value = fluid_props.get_hmass(p_data[0], T_data[0])
                    
                    # Create pressure range for theoretical line
                    p_range = np.linspace(max(0.1, p_data.min() - 0.5), 
                                        p_data.max() + 0.5, 100)
                    
                    # Plot theoretical isenthalpic line
                    self._plot_isenthalpic_line(ax, fluid_props, p_range, h_value,
                                              color=color, linewidth=2, alpha=0.8,
                                              label='EOS' if not legend_added else None)
                    ...
                    print(f"Error plotting theoretical line: {e}")
    ...
    def plot_jt_coefficients_comparison(self, save_figure=True):
        ...
        Creates detailed comparison plots for publication.
        """
        # Get all fluid data
